...
describe("extendMetadataWithV1ObjectMeta", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("extends Metadata with V1ObjectMeta and adds import when CRD defines metadata schema", () => {
    const lines = [
      "// This file is auto-generated by mockPackage, do not edit manually",
      'import { GenericKind, RegisterKind } from "mockPackage";',
      "",
    ...
    const crdWithMetadata = {
      ...
      spec: {
        group: "uds.dev",
        scope: "Cluster",
        names: { kind: "ClusterConfig", plural: "clusterconfig" },
        versions: [
          {
            name: "v1alpha1",
            served: true,
    ...
    } as unknown as CustomResourceDefinition;

    const result = postProcessingModule.extendMetadataWithV1ObjectMeta(
      lines,
      crdWithMetadata,
      "v1alpha1",
    );

    // Interface should now extend V1ObjectMeta
    expect(
      result.some(line => line.trim() === "export interface Metadata extends V1ObjectMeta {"),
    ).toBe(true);

    // The name property should still be present
    expect(result.some(line => line.includes("name?: Name;"))).toBe(true);

    // Import for V1ObjectMeta should be added after existing imports
    const genericImportIdx = result.findIndex(line =>
      line.includes('import { GenericKind, RegisterKind } from "mockPackage";'),
    );
    const v1ObjectMetaImportIdx = result.findIndex(line =>
      line.includes('import type { V1ObjectMeta } from "@kubernetes/client-node";'),
    );

    expect(v1ObjectMetaImportIdx).toBeGreaterThan(genericImportIdx);
  ...
  it("does nothing when CRD does not define metadata schema", () => {
    const lines = [
      "// header",
    ...
    const result = postProcessingModule.extendMetadataWithV1ObjectMeta(
      lines,
      mockCRDResults[0].crd as CustomResourceDefinition,
      mockCRDResults[0].version,
    );
